package starling.utils
{
    ...
    public class Polygon extends DisplayObject
    {
    	...
    	private static var sPoint1:Point = new Point();
		private static var sPoint2:Point = new Point();
		private static var sBool:Boolean = false;
        ...
        public override function hitTest(localPoint:Point, forTouch:Boolean=false):DisplayObject
        {
    		...
    		{
				// using a point inclusion in polygon test for a more precise result
				var i:int;
				var j:int;
				sBool = false;
				for (i=0, j=mNumEdges-1; i<mNumEdges; j=i++) 
				{
					mVertexData.getPosition(i, sPoint1);
					mVertexData.getPosition(j, sPoint2);
					if (((sPoint1.y > localPoint.y) != (sPoint2.y > localPoint.y)) && (localPoint.x < (sPoint2.x - sPoint1.x) * (localPoint.y - sPoint1.y) / (sPoint2.y - sPoint1.y) + sPoint1.x))
						sBool = !sBool;
				}
				
				if (sBool) return this;
				else return null;
			}
            else return null;
        }
